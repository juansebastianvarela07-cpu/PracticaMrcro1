...
;--- BITS DE CONFIGURACIÓN--
 ...
 CONTADOR_ADC: DS 1 ; TEMPORIZADOR SECUNDARIO PARA LANZAR LA LECTURA
    ...
    ;===== CONTROL DE DISPLAY MULTIPLEXADOS ======
        DISP_SEL: DS 1 ; BIT 0: 0 = UNIDADES (((PORTD) and 0FFh), 4, a), 1 = DECENAS (((PORTD) and 0FFh), 5, a)
...
Inicio:
    ...
    ;((PORTD) and 0FFh), 4, a: SELECTOR DISPLAY 1 (UNIDADES - ACTIVO EN ALTO
    ;((PORTD) and 0FFh), 5, a; SELECTOR DISPLAY 2 (DECENAS) - ACTIVO EN ALTO

    ;PUERTOS E: SALIDAS (ALARMA Y VENTILADOR)
    BCF TRISE, 0, a ; ((PORTE) and 0FFh), 0, a COMO SALIDA (LED ALARMA)
    BCF LATE, 0, a ; ((PORTE) and 0FFh), 0, a = 0 (LED APAGADO INICIALMENTE)
    BCF TRISE, 1, a ; ((PORTE) and 0FFh), 1, a COMO SALIDA (CONTROL VENTILADOR
    BCF LATE, 1, a ; ((PORTE) and 0FFh), 1, a = 0 (VENTILADOR APAGADO INICIALMENTE)

    ;PUERTO A: ENTRADA ANALÓGICA (SENSOR LM35)
    BSF TRISA,0,a ;A0 COMO ENTRADA (TRIS = 1)
    ; ((PORTA) and 0FFh), 0, a ES ENTRADA ANALÓGICA DEL ADC, NO SE INICIALIZA LAT

    ;PUERTO B: ENTRADAS DIGITALES (PULSADORES PARA INTERRUPCIONES)
    BSF TRISB, 0,a ; ((PORTB) and 0FFh), 0, a COMO ENTRADA (PULSADORES ((PORTB) and 0FFh), 0, a - ALARMA)
    BSF TRISB, 1, a ; ((PORTB) and 0FFh), 1, a COMO ENTRADA (PULSADORES ((PORTB) and 0FFh), 1, a - VENTILADOR)
    BSF TRISB, 2,a ; ((PORTB) and 0FFh), 2, a COMO ENTRADA (PULSADORES ((PORTB) and 0FFh), 2, a - °C/°F)
    ; ((PORTB) and 0FFh), 0, a, ((PORTB) and 0FFh), 1, a, ((PORTB) and 0FFh), 2, a PUEDEN TENER PULL-UPS INTERNOS

    ; ============================================================
    ; SECCIÓN 4: CONFIGURAR MÓDULO ADC
    ; ============================================================
    ; Analógico-Digital Converter: Convierte voltaje analógico a valor digital
    ; Entrada: ((PORTA) and 0FFh), 0, a conectado a sensor LM35
    ; Rango: 0 a 5V ? 0 a 1023 (10 bits) (SEGÚN CLAUDE)

    ; Registro ADCON1: Configurar pines analógicos vs digitales
    ; ((ADCON1) and 0FFh), 3, a:((ADCON1) and 0FFh), 0, a = 1110 ? ((PORTA) and 0FFh), 0, a (((PORTA) and 0FFh), 0, a) es analógico, resto digital
    ; ((ADCON1) and 0FFh), 5, a:((ADCON1) and 0FFh), 4, a = 00 ? Vref+ = VDD (5V), Vref- = VSS (0V)
    ; Valor: 0x0E = 00001110B (SEGÚN CLAUDE)


    MOVLW 0x0E ; CARGA 0x0E (14 DECIMAL) EN W
    MOVWF ADCON1, a ; ADCON1 = 0x0E (((PORTA) and 0FFh), 0, a ANALÓGICO)

    ; Registro ADCON2: Configurar tiempo de conversión
    ; ((ADCON2) and 0FFh), 7, a = 1 (bit 7): Resultado justificado a la DERECHA
    ...
    ; CH3:CH0 = 0000 ? Canal 0 (((PORTA) and 0FFh), 0, a = ((PORTA) and 0FFh), 0, a)
    ; ((ADCON0) and 0FFh), 1, a = 0 ? ADC no activo (se dispara desde ISR_TMR0)
    ; ((ADCON0) and 0FFh), 0, a = 1 ? ADC habilitado
    ...
    ; La diferencia está en el bit 6 (((T0CON) and 0FFh), 6, a):
    ...
    ; Bit 7 (((T0CON) and 0FFh), 7, a) = 1: Timer encendido
    ; Bit 6 (((T0CON) and 0FFh), 6, a) = 1 (ORIGINAL INCORRECTO): Modo 8 bits
    ; Bit 6 (((T0CON) and 0FFh), 6, a) = 0 (CORRECCIÓN): Modo 16 bits ? USAR ESTO
    ; Bit 5 (TOCS) = 0: Reloj interno (Fosc/4)
    ; Bit 4 (TOSE) = 0: Incrementa en flanco ascendente
    ; Bit 3 (((T0CON) and 0FFh), 3, a) = 1: Prescaler habilitado
    ...
    BCF INTCON2, 7, a ; ((INTCON2) and 0FFh), 7, a = 0: Habilita pull-ups internos de PORTB
                         ; NECESARIO SI LOS PULSOS VAN A GND CON PULL-DOWN

    ;ESCOGE FLANCO DE SIPARO PARA LAS INTERRUPCIONES EXTERNAS
    BSF INTCON2, 6, a ; ((INTCON2) and 0FFh), 6, a = 1 ((RCON) and 0FFh), 1, a FLANCO ASCENDENTES (0-1)
    BSF INTCON2, 5, a
    BSF INTCON2, 4, a

    ; CONSULTAR PULSADORES ((RCON) and 0FFh), 1, a SI VAN A VDD (PULSADORES A NIVEL ALTO)
    ...
    BCF INTCON, 1, a ; ((INTCON) and 0FFh), 1, a = 0 (BANDERA DE ((PORTB) and 0FFh), 0, a)
    BCF INTCON3, 0, a ; ((INTCON3) and 0FFh), 0, a = 0 (BANDERA DE ((PORTB) and 0FFh), 1, a)
    BCF INTCON3, 1, a ; ((INTCON3) and 0FFh), 1, a = 0 (BANDERA DE ((PORTB) and 0FFh), 0, a)
    BCF INTCON, 2, a ; ((INTCON) and 0FFh), 2, a = 0 (BANDERA DE TIMER0)
    BCF PIR1, 6, a ; AD0IF = 0 (BANDERA DE ADC TERMINADO)

    ;HABIITAR INTERRUPCIONES INDIVIDUALES
    BSF INTCON,4, a ; ((INTCON) and 0FFh), 1, a = 1 (HABILITAR ((PORTB) and 0FFh), 0, a)
    BCF INTCON3, 3, a ; ((INTCON3) and 0FFh), 0, a = 1 (HABILITAR ((PORTB) and 0FFh), 1, a)
    BSF INTCON3, 4, a ; ((INTCON3) and 0FFh), 1, a = 1 (HABILITAR ((PORTB) and 0FFh), 0, a)
    BSF INTCON,2, a ; ((INTCON) and 0FFh), 2, a = 1 (HABILITAR TIMER0)
    BSF PIE1, 6, a ; AD0IF = 1 (HABILITAR ADC)

    ;HABILITAR INTERRUPCIONES GLOBALES
    BSF INTCON, 6, a ; ((INTCON) and 0FFh), 6, a = 1 (PERIPHERAL INTERRUP ENABLE)
    ; PERMITE QUE LOS PERIFÉRICOS CAUSEN INTERRUPCIONES
    BSF INTCON, 7, a ; ((INTCON) and 0FFh), 7, a = 1 (GLOBAL INTERRUPT ENABLE)

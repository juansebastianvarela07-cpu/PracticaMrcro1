...
Inicio:
    ...
    BCF TRISE, 0 ; ((PORTE) and 0FFh), 0, a COMO SALIDA (LED ALARMA)
    BCF LATE, 0 ; ((PORTE) and 0FFh), 0, a = 0 (LED APAGADO INICIALMENTE)
    BCF TRISE, 1 ; ((PORTE) and 0FFh), 1, a COMO SALIDA (CONTROL VENTILADOR
    BCF LATE, 1 ; ((PORTE) and 0FFh), 1, a = 0 (VENTILADOR APAGADO INICIALMENTE)

    ;PUERTO A: ENTRADA ANALÓGICA (SENSOR LM35)
    BSF TRISA, 0 ;A0 COMO ENTRADA (TRIS = 1)
    ...
    BSF TRISB, 0 ; ((PORTB) and 0FFh), 0, a COMO ENTRADA (PULSADORES ((PORTB) and 0FFh), 0, a - ALARMA)
    BSF TRISB 1 ; ((PORTB) and 0FFh), 1, a COMO ENTRADA (PULSADORES ((PORTB) and 0FFh), 1, a - VENTILADOR)
    BSF TRISB, 2 ; ((PORTB) and 0FFh), 2, a COMO ENTRADA (PULSADORES ((PORTB) and 0FFh), 2, a - °C/°F)
    ...
    MOVWF ADCON1 ; ADCON1 = 0x0E (((PORTA) and 0FFh), 0, a ANALÓGICO)

...
PSECT udata_acs
    ...
    ESTADO_ALARMA: DS 1 ; 0 = APAGADA, 1 = ENCENDIDA (((PORTB) and 0FFh), 0, a)
    ESTADO_VENTILADOR: DS 1 ; 0 = APAGADO, 1 = ENCENDIDO (((PORTB) and 0FFh), 1, a)
    MODO_UNIDAD: DS 1 ; 0 = °CELSIUS, 1 = °FAHRENHEIT (((PORTB) and 0FFh), 2, a)
    ...
    ESTADO_AUTO: DS 1 ; BANDERA: 1 = TEMP >= 28, 0 = TEMP < 28
;===============================================
; VECTOR DE RESET
;===============================================
...
; ======= ((PORTB) and 0FFh), 0, a - LED ALARMA ============
    BTFSS INTCON, 1, a ; ¿SE ACTIVO LA BANDERA DE ((PORTB) and 0FFh), 1, a?
    GOTO ISR_INT1
    BTG LATC,0, a ; INVIERTE ESTADO FISICO DEL LED (((PORTC) and 0FFh), 0, a)
    BTG ESTADO_ALARMA, 0, a ; INVIERTE ESTADO EN RAM
    BCF INTCON, 1, a ; LIMPIA BANDERA DE ((PORTB) and 0FFh), 0, a

ISR_INT1:
; ======= ((PORTB) and 0FFh), 1, a - VENTILADOR ============
    BTFSS INTCON3, 0, a ; SE ACTIVO BANDERA ((PORTB) and 0FFh), 1, a?
    GOTO ISR_INT2
    BTG LATC, 1, a ; INVIERTE EL ESTADO DEL TRANSISTOR (((PORTC) and 0FFh), 1, a)
    BTG ESTADO_VENTILADOR, 0, a ; INVIERTE ESTADO EN LA RAM
    BCF INTCON3, 0, a ; LIMPIA LA BANDERA DE ((PORTB) and 0FFh), 1, a

ISR_INT2:
; ======= ((PORTB) and 0FFh), 2, a - BOTON C/F ============
    BTFSS INTCON3, 1, a ; SE ACTIVO LA BANDERA ((PORTB) and 0FFh), 2, a?
    GOTO ISR_TMR0
    BTG MODO_UNIDAD, 0, a ; CAMBIA ENTRE 0(°C) y 1(°F)
    BSF ACTUALIZAR, 0, a ; AVISA REFRESCO DEL DISPLAY
    BCF INTCON3, 1, a ; LIMPIA LA BANDERA DEL ((PORTB) and 0FFh), 2, a
...
INICIO:
    ...
    ; ============================================================
    ; PUERTOS A
    BSF TRISA, 0, a ; ((PORTA) and 0FFh), 0, a: ENTRADA ANALOGICA (LM35)
    BCF TRISA, 1, a ; ((PORTA) and 0FFh), 1, a: SALIDA (TRANSISTOR DECENAS)
    ; PUERTOS B
    BSF TRISB, 0, a ; ((PORTB) and 0FFh), 0, a: ENTRADA (BOTON ALARMA)
    BSF TRISB, 1, a ; ((PORTB) and 0FFh), 1, a: ENTRADA (BOTON VENTILADOR)
    BSF TRISB, 2, a ;((PORTB) and 0FFh), 2, a: ENTRADA (BOTON C/F)
    ; PUERTOS C
    BCF TRISC, 0, a ; ((PORTC) and 0FFh), 0, a: SALIDA (LED ALARMA)
    BCF TRISC, 1, a ; ((PORTC) and 0FFh), 1, a: SALIDA (TRANSISTOR VENTILADOR)
    BCF TRISC, 2, a ; ((PORTC) and 0FFh), 2, a: SALIDA (TRANSISTOR UNIDADES)

    MOVLW 10000000B
    MOVWF TRISD, a ; ((PORTD) and 0FFh), 0, a-((PORTD) and 0FFh), 6, a: SALIDAS PARA LOS 7 SEGMENTOS

    MOVLW 00000111B
    MOVWF CMCON, a ; APAGAR COMPARADORES

    BSF TRISA, 0, a ; PIN ((PORTA) and 0FFh), 0, a CONFIGURADO COMO ENTRADA
    ; === CONFIGURACION DEL CONVERSOR ANALOGO-DIGITAL (ADC) ===
    MOVLW 00001110B
    MOVWF ADCON1, a ; ASIGNA UNICAMENTE ((PORTA) and 0FFh), 0, a COMO ANALOGO, RESTO COMO DIGITALES
    MOVLW 00010010B
    MOVWF ADCON2, a ; JUSTIFICA EL RESULTADO A LA IZQUIERDA Y CONFIGURA LOS TIEMPOS
    MOVLW 00000001B
    MOVWF ADCON0, a ; SELECCIONA EL CANAL ((PORTA) and 0FFh), 0, a Y ENCIENDE ELECTRICAMENTE EL MODULO ADC
    ...
    BSF INTCON, 4, a ; HABILITA ((PORTB) and 0FFh), 0, a (ALARMA)
    BSF INTCON3, 3, a ; HABILITA ((PORTB) and 0FFh), 1, a (VENTILADOR)
    BSF INTCON3, 4, a ; HABILITA ((PORTB) and 0FFh), 2, a (GRADOS)
    BSF INTCON, 5, a ; HABILITA LA INTERRUPCION DEL TIMER0
    BSF T0CON, 7, a ; ENCIENDE OFICIALMENTE EL MODULO TIMER0
    BSF INTCON, 7, a ; ENCIENDE EL INTERRUPTOR GLOBAL DE INTERRUPCIONES (((INTCON) and 0FFh), 7, a)
...
ESPERAR_ADC:
    BTFSC ADCON0, 1, a ; VERIFICA CONSTANTEMENTE SI EL BIT ((ADCON0) and 0FFh), 1, a SE PUSO EN CERO
    GOTO ESPERAR_ADC

    MOVF ADRESH, W, a
    ADDWF ADRESH, W, a ; MULTIPLICA ((RCON) and 0FFh), 1, a DOS SUMANDO EL REGISTRO CONSIGO MISMO
...
REVISAR_28:
    ...
    BTFSS STATUS, 0, a ; TEMP >= 28?
    GOTO TEMP_BAJA ; SI ES MENOR A 28, SALTA

TEMP_ALTA:
    BTFSC ESTADO_AUTO, 1, a ; EL SISTEMA SABE SI HAY CALOR?
    GOTO CELSIUS ; SI: NO HACE NADA
    BSF ESTADO_AUTO, 1, a ; NO: APENAS CRUZA 28°C ACTUALIZA
    BSF LATC, 0, a ; ENCENDIDO AUTO DEL LED
    BSF LATC, 1, a ; ENCENDIDO AUTO DEL VENTILADOR
    GOTO CELSIUS
TEMP_BAJA:
    BTFSS ESTADO_AUTO, 1, a ; EL SISTEMA SABE SI HAY FRIO?
    GOTO CELSIUS ; SÍ: NO HACE NADA
    BCF ESTADO_AUTO, 1, a ; NO: BAJO DE 28°C. ACTUALIZA
    BCF LATC, 0, a ; APAGADO AUTO DEL LED
    BCF LATC, 1, a ; APAGADO AUTO DEL VENTILADOR

ACTUALIZAR_RC:
    ; --- LÓGICA DEL LED(((PORTC) and 0FFh), 0, a) ---
    BTFSC ESTADO_AUTO, 0, a ; SI AUTO = 1, ENCENDER
    GOTO ENCENDER_LED
    BTFSC ESTADO_ALARMA, 0, a ; SI MANUAL (((PORTB) and 0FFh), 0, a) = 1, ENCENDER
    GOTO ENCENDER_LED
    BCF LATC, 0, a ; SI AMBOS SON 0, APAGAR PIN ((PORTC) and 0FFh), 0, a
    GOTO REVISAR_VENTILADOR

ENCENDER_LED:
    BSF LATC, 0, a ; ENCIENDE PIN ((PORTC) and 0FFh), 0, a

REVISAR_VENTILADOR:
    ; --- LÓGICA DEL VENTILADOR(((PORTC) and 0FFh), 1, a) ---
    BTFSC ESTADO_AUTO, 0, a ; SI AUTO = 1, ENCENDER
    GOTO ENCENDER_VENT
    BTFSC ESTADO_VENTILADOR, 0, a ; SI MANUAL (((PORTB) and 0FFh), 1, a) = 1, ENCENDER
    GOTO ENCENDER_VENT
    BCF LATC, 1, a ; SI AMBOS SON 0, APAGAR ((PORTC) and 0FFh), 1, a
    GOTO CELSIUS

ENCENDER_VENT:
    BSF LATC, 1, a ; ENCIENDE ((PORTC) and 0FFh), 1, a
    GOTO CELSIUS
...
BCD:
    ...
    BNC FIN_BCD ; SI ((SSPSTAT) and 0FFh), 5, a NEGATIVO SALTA AL FINAL DEL BCD
...
MULTIPLEXAR:
    BCF LATA, 1, a ; APAGA TRANSISTOR DE DECENAS (((PORTA) and 0FFh), 1, a)
    BCF LATC, 2, a ; APAGA TRANSISTOR DE UNIDADES (((PORTC) and 0FFh), 2, a)

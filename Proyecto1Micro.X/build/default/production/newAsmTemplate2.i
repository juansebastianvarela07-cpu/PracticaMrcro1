...
# 1 "C:\\Program Files\\Microchip\\xc8\\v4.00\\pic\\include/xc.inc" 1 3
...
auto_size SET 0
ENDM
# 6 "C:\\Program Files\\Microchip\\xc8\\v4.00\\pic\\include/xc.inc" 2 3
...
Inicio:
    ...
    BCF TRISC, 0, a ; PIN ((PORTC) and 0FFh), 0, a (PIN 15) COMO SALIDA (TRANSISTOR DECENAS)
    BCF TRISC, 1, a ; PIN ((PORTC) and 0FFh), 1, a (PIN 16) COMO SALIDA (TRANSISTOR UNIDADES)
    CLRF LATC, a ; APAGA AMBOS TRANSISTORES PARA INICIAR

    ; PUERTO E: SALIDAS PARA ALARMA Y VENTILADOR
    BCF TRISE, 0, a ; PIN ((PORTE) and 0FFh), 0, a COMO SALIDA (LED ALARMA)
    BCF LATE, 0, a ; INICIA EL LED DE ALARMA APAGADO
    BCF TRISE, 1, a ; PIN ((PORTE) and 0FFh), 1, a COMO SALIDA (VENTILADOR)
    ...
    BSF TRISB, 0, a ; PIN ((PORTB) and 0FFh), 0, a COMO ENTRADA (BOTON 1 - ((PORTB) and 0FFh), 0, a - ALARMA)
    BSF TRISB, 1, a ; PIN ((PORTB) and 0FFh), 1, a COMO ENTRADA (BOTON 2 - ((PORTB) and 0FFh), 1, a - VENTILADOR)
    BSF TRISB, 2, a ; PIN ((PORTB) and 0FFh), 2, a COMO ENTRADA (BOTON 3 - ((PORTB) and 0FFh), 2, a - UNIDAD °C/°F)

    ; ============================================================
    ; SECCION 4: CONFIGURAR MODULO ADC PARA EL SENSOR DE TEMPERATURA
    ; ============================================================
    MOVLW 0x0E ; VALOR 0x0E (14 DECIMAL)
    MOVWF ADCON1, a ; CONFIGURA UNICAMENTE ((PORTA) and 0FFh), 0, a COMO ANALOGO, EL RESTO DIGITALES

    MOVLW 10100010B ; JUSTIFICADO A LA DERECHA, FOSC/32
    MOVWF ADCON2, a ; GUARDA CONFIGURACION EN ADCON2

    MOVLW 00000001B ; SELECCIONA CANAL ((PORTA) and 0FFh), 0, a Y ENCIENDE EL MODULO (((ADCON0) and 0FFh), 0, a=1)
    ...
    BSF INTCON2, 6, a ; ((PORTB) and 0FFh), 0, a SE ACTIVA ((RCON) and 0FFh), 1, a FLANCO DE SUBIDA (0 A 1)
    BSF INTCON2, 5, a ; ((PORTB) and 0FFh), 1, a SE ACTIVA ((RCON) and 0FFh), 1, a FLANCO DE SUBIDA (0 A 1)
    BCF INTCON2, 4, a ; ((PORTB) and 0FFh), 2, a SE ACTIVA ((RCON) and 0FFh), 1, a FLANCO DE BAJADA (1 A 0, BOTON A GND)
    ...
    BCF INTCON3, 1, a ; LIMPIA BANDERA DE ((PORTB) and 0FFh), 2, a
    ...
    BSF INTCON, 6, a ; HABILITA INTERRUPCIONES PERIFERICAS (((INTCON) and 0FFh), 6, a)
    BSF INTCON, 7, a ; HABILITA INTERRUPCIONES GLOBALES (((INTCON) and 0FFh), 7, a)
...
Loop_Principal:
    ...
    BSF ADCON0, 1, a ; PONE EN 1 EL BIT ((ADCON0) and 0FFh), 1, a/((ADCON0) and 0FFh), 1, a PARA INICIAR LA LECTURA
ESPERAR_ADC:
    BTFSC ADCON0, 1, a ; REVISA SI EL HARDWARE PUSO EL BIT ((ADCON0) and 0FFh), 1, a/((ADCON0) and 0FFh), 1, a EN 0
    ...
    ; DIVISION ((RCON) and 0FFh), 1, a 2 (ROTACION DE BITS) PARA CONVERTIR EL VOLTAJE EN GRADOS CELSIUS
    BCF STATUS, 0, a ; LIMPIA EL BIT DE ACARREO (((STATUS) and 0FFh), 0, a) ((RCON) and 0FFh), 1, a SEGURIDAD
...
CALCULAR_FAHRENHEIT:
    ...
    MULLW 9 ; MULTIPLICA ((RCON) and 0FFh), 1, a 9
...
RESTAR_5:
    ...
    BNC FIN_DIV5 ; SI EL RESULTADO ES NEGATIVO (NO ((STATUS) and 0FFh), 0, a), TERMINO LA DIVISION
...
BUCLE_RESTAR_10:
    ...
    BNC FIN_BCD ; SI ((SSPSTAT) and 0FFh), 5, a NEGATIVO, SIGNIFICA QUE YA NO QUEDAN MAS DECENAS
...
;================================================
Rutina_ISR:
    BTFSC INTCON, 1, a ; ¿SE ACTIVO LA BANDERA DE ((PORTB) and 0FFh), 0, a?
    GOTO ISR_INT0 ; VE A LA SUBRUTINA DE ((PORTB) and 0FFh), 0, a
    BTFSC INTCON3, 0, a ; ¿SE ACTIVO LA BANDERA DE ((PORTB) and 0FFh), 1, a?
    GOTO ISR_INT1 ; VE A LA SUBRUTINA DE ((PORTB) and 0FFh), 1, a
    BTFSC INTCON3, 1, a ; ¿SE ACTIVO LA BANDERA DE ((PORTB) and 0FFh), 2, a?
    GOTO ISR_INT2 ; VE A LA SUBRUTINA DE ((PORTB) and 0FFh), 2, a

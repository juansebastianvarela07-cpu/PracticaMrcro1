...
Inicio:
    ...
    BSF TRISA, 0, c ; ((PORTA) and 0FFh), 0, a como entrada (Potenciómetro)

    ; 3. Configuración del Módulo ADC
    MOVLW 0x0E ; 00001110: ((PORTA) and 0FFh), 0, a como análogo, resto digitales. Vref = VDD/VSS
    MOVWF ADCON1, c

    ; ((ADCON2) and 0FFh), 7, a=0 (Izquierda), ACQT=100 (4 TAD), ADCS=010 (FOSC/32)
    MOVLW 00010010B
    MOVWF ADCON2, c

    ; ((ADCON0) and 0FFh), 5, a:((ADCON0) and 0FFh), 2, a = 0000 (((PORTA) and 0FFh), 0, a), ((ADCON0) and 0FFh), 0, a = 1
    MOVLW 00000001B
    MOVWF ADCON0, c

Loop_ADC:
    ; 4. Iniciar la conversión A/D
    BSF ADCON0, 1, c ; Poner en 1 el bit ((ADCON0) and 0FFh), 1, a/((ADCON0) and 0FFh), 1, a (Bit 1 del registro ADCON0)

Wait_ADC:
    BTFSC ADCON0, 1, c ; Monitorear el bit ((ADCON0) and 0FFh), 1, a/((ADCON0) and 0FFh), 1, a hasta que el hardware lo ponga en 0
    GOTO Wait_ADC
